...
sub test_logDirName {
    ...
    $this->assert_str_equals( "dummy", $ks->logDirName(),
        "Bad configured log dir" );
}
...
sub test_indexPath {
    ...
    $this->assert_str_equals( "dummy", $ks->indexPath(),
        "Bad configured index dir" );
}
...
sub test_skipWebs {
    ...
    %webs = $ks->skipWebs();
    foreach $a_web (@config_webs) {
        $this->assert( $webs{$a_web},
            "Web $a_web not skipped in config (comma, no space)." );
    }
    ...
    foreach $a_web (@config_webs) {
        $this->assert( $webs{$a_web},
            "Web $a_web not skipped in config (additional spaces)." );
    }
    ...
    foreach $a_web (@config_webs) {
        $this->assert( $webs{$a_web},
            "Web $a_web not skipped in config (only space)." );
    }
    ...
    foreach $a_web (@config_webs) {
        $this->assert( $webs{$a_web},
            "Web $a_web not skipped in config (many space)." );
    }
...
sub test_skipAttachments {
    ...
    foreach my $a_att (@config_atts) {
        $this->assert( $atts{$a_att},
            "Attachment $a_att not skipped in config." );
    }
}
