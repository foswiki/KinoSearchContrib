...
THIS
    ...
    Foswiki::Func::saveTopic( $this->{test_web}, "TopicWithForm", $meta,
        'Foo' );
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "TopicWithForm",
        "Wrong topic for form." );

    $docs = $search->docsForQuery("form:DoNotExistForm");
    $hit  = $docs->fetch_hit_hashref;
    $this->assert( !defined($hit),
        "Hit for DoNotExistForm found. Should be undefined!" );
}
...
sub test_updateIndex {
    ...
    my $hit    = $docs->fetch_hit_hashref;
    $this->assert( !defined($hit),
        "Hit for updatepoint found. Should be undefined!" );

    # Now I do some changes
    Foswiki::Func::saveTopicText( $this->{test_web}, "NewOrChangedTopicUpdate",
        <<'HERE');
Just an example topic
Keyword: updatedpoint
HERE
    Foswiki::Func::saveTopicText( $this->{test_web}, "NewOrChangedTopicUpdate2",
        <<'HERE');
Just an example topic
...
HERE
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "NewOrChangedTopicUpdate",
        "Wrong topic for update topic." );
    ...
    $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "NewOrChangedTopicUpdate2",
        "Wrong topic for update2 topic." );

    # Lets delete a topic
    Foswiki::Func::moveTopic(
        $this->{test_web}, "NewOrChangedTopicUpdate",
    ...
    $hit  = $docs->fetch_hit_hashref;
    $this->assert( !defined($hit),
        "Hit for deleted topic found. Should be undefined!" );

    # Now let's add an attachment
    Foswiki::Func::saveAttachment( $this->{test_web},
        "NewOrChangedTopicUpdate2", "Simple_example.doc",
    ...
    $topic = $hit->{topic};
    $this->assert_str_equals( $topic, "NewOrChangedTopicUpdate2",
        "Wrong topic for changed attach." );
}
...
sub test_attachmentsOfTopic {
    ...
    @atts =
      $ind->attachmentsOfTopic( $this->{test_web}, "TopicWithoutAttachment" );
    $this->assert( !@atts, "Atts should be undefined." );

    @atts =
      $ind->attachmentsOfTopic( $this->{test_web}, "TopicWithWordAttachment" );
    $this->assert( @atts, "Atts should be defined." );
    $this->assert_str_equals( $atts[0]->{'name'},
        "Simple_example.doc", "Attachment name not O.K." );
}
...
sub test_changedTopics {
    ...
    # Now I do a change
    Foswiki::Func::saveTopicText( $this->{test_web}, "NewOrChangedTopic",
        <<'HERE');
Just an example topic
Keyword: startpoint
HERE
    ...
    foreach $change (@changes) {
        $this->assert_str_equals( $change, "NewOrChangedTopic",
            "Last change not detected." );
        last;
    }
    ...
    # The first change should be the one I just did.
    foreach $change (@changes) {
        $this->assert_str_equals( $change, "TopicWithoutAttachment",
            "Last change not detected." );
        last;
    }
}
...
sub test_removeTopics {
    ...
    $hit  = $docs->fetch_hit_hashref;
    $this->assert( !defined($hit),
        "Hit for startpoint found. Should be removed" );
...
HERE
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "NewTopic",
        "Wrong topic for update topic." );
}
...
sub test_websToIndex {
    ...
    $this->assert( grep( /$this->{test_web}/, @webs ),
        "Test web not included." );
}
...
sub test_fieldNamesFileName {
    ...
    $this->assert_str_equals( $ind->indexPath() . "/.formFieldNames",
...
sub test_indexTopic {
    ...
    Foswiki::Func::saveTopicText( $this->{test_web}, "TopicWithExcelAttachment",
        <<'HERE');
Just an example topic with MS Excel
Keyword: spreadsheet
HERE
    Foswiki::Func::saveAttachment( $this->{test_web},
        "TopicWithExcelAttachment", "Simple_example.xls",
    ...
    # Indexing
    $ind->indexTopic( $indexer, $this->{test_web}, "TopicWithExcelAttachment",
        %fldNames );
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "TopicWithExcelAttachment",
...
sub test_indexAttachment {
    ...
    Foswiki::Func::saveTopicText( $this->{test_web}, "TopicWithPdfAttachment",
        <<'HERE');
Just an example topic with PDF
Keyword: spreadsheet
HERE
    Foswiki::Func::saveAttachment( $this->{test_web}, "TopicWithPdfAttachment",
        "Simple_example.pdf",
    ...
    # Indexing
    my @allAttachments =
      $ind->attachmentsOfTopic( $this->{test_web}, "TopicWithPdfAttachment" );
    my $attachment = $allAttachments[0];
    $ind->indexAttachment( $indexer, $this->{test_web},
        "TopicWithPdfAttachment", $attachment );
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "TopicWithPdfAttachment",
        "Wrong topic for tile." );
    $this->assert_str_equals(
        $this->{test_web} . "TopicWithPdfAttachment" . $attachment->{'name'},
        $hit->{'id_topic'}, "ID topic not O.K." );
    $this->assert_str_equals( $hit->{'name'}, $attachment->{'name'},
        "Name not O.K." );
    $this->assert_str_equals( $hit->{'author'}, $attachment->{'user'},
        "User not O.K." );
    $this->assert_str_equals(
        $hit->{'version'},
        $attachment->{'version'},
    ...
    $this->assert_str_equals( $hit->{'attachment'}, "yes",
        "Attachment not set to yes." );
...
sub test_saveUpdateMarker {
    ...
    $this->assert_str_equals( $start_time, $red_time,
        "Red time does not fit saved time." );
}
...
sub test_tripFirstchar {
    ...
    my $result =
      Foswiki::Contrib::KinoSearchContrib::Index::tripFirstchar("TTTTheTopic");
...
sub test_splitTheTopicName {
    ...
    my $result = Foswiki::Contrib::KinoSearchContrib::Index::splitTheTopicName(
        "TheTopic--NNNName");
...
sub test_splitTopicName {
    ...
    my $result = Foswiki::Contrib::KinoSearchContrib::Index::splitTopicName(
        "TheTopicName   IsKnown");
...
sub _testForWordInAttachment {
    ...
    Foswiki::Func::saveTopicText( $this->{test_web}, "TopicWithSpecialFile",
        <<'HERE');
Just an example topic.
Keyword: Superspecific
HERE
    Foswiki::Func::saveAttachment( $this->{test_web}, "TopicWithSpecialFile",
        $file, { file => $this->{attachmentDir} . $file } );
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "TopicWithSpecialFile",
        "Wrong topic for $word: Is = $topic but should be TopicWithSpecialFile"
    );
}
...
sub _indexOK {
    ...
    my $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "TopicWithoutAttachment",
        "Wrong topic for startpoint." );
    ...
    $topic = $hit->{topic};
    $topic =~ s/ .*//;
    $this->assert_str_equals( $topic, "TopicWithWordAttachment",
        "Wrong topic for redmond." );
}
