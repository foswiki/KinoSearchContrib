...
package Foswiki::Contrib::KinoSearchContrib::Search;
use strict;
...
sub search {
    ...
    $self->{Debug} = $debug || $Foswiki::cfg{KinoSearchContrib}{Debug} || 0;
...
"searching:'$search' from:'$webName.$topicName' in web:'$websStr' by:'$remoteUser'"
      );
...
#$search =~ s/^\sand\s//go;       # if new search starts with and, it should be removed
    ...
    my ( $tmplHead, $tmplSearch, $tmplTable, $tmplNumber, $tmplTail ) =
      split( /%SPLIT%/, $tmpl );
    $tmplHead =
      Foswiki::Func::expandCommonVariables( $tmplHead, $topicName, $webName );
    $tmplSearch =
      Foswiki::Func::expandCommonVariables( $tmplSearch, $topicName, $webName );
    $tmplNumber =
      Foswiki::Func::expandCommonVariables( $tmplNumber, $topicName, $webName );
    $tmplTail =
      Foswiki::Func::expandCommonVariables( $tmplTail, $topicName, $webName );
    ...
    if ( !$tmplTail ) {
        ...
        $result .=
          "Incorrect format of kinosearch.tmpl (missing %SPLIT% parts)";
        $result .= "</body></html>";
        return;
    }
...
# if only attachments are displayed, even if configured, then the message is not shown
    if ( ($searchAttachmentsOnly) && ( $usersearch !~ "attachment:yes" ) ) {
        ...
        my $attachmentsOnlyLabel =
          $Foswiki::cfg{KinoSearchContrib}{AttachmentsOnlyLabel}
          || "Show only attachments";
        $tmplSearch =~
s/%SEARCHATTACHMENTSONLY%/<a href="%SCRIPTURLPATH%\/kinosearch\/$webName\/?search=$tempVal\%20\%2Battachment:yes">$attachmentsOnlyLabel<\/a>/go;
        $tmplSearch =
          Foswiki::Func::expandCommonVariables( $tmplSearch, $topicName,
            $webName );
    }
...
# just for cleaning if the preference isn't set, or already displaying only attachments
    ...
    if ( !$noheader ) {
        my $bgcolor =
          Foswiki::Func::getPreferencesValue( "WEBBGCOLOR", $webName )
          || "#FFFFFF";
        ...
        $beforeText =~ s/%WEB%/ /go;
        $beforeText =
          Foswiki::Func::expandCommonVariables( $beforeText, $topicName,
            $webName );
    ...
    while ( my $hit = $docs->fetch_hit_hashref ) {
     ...
     # For partial name search of topics, just hold the first part of the string
        ...
        if ($rss) {
            $outString =
              $self->renderRssStringFor( $hit, $repeatText, $nosummary,
                $showlock );
        }
        else {
            $outString =
              $self->renderHtmlStringFor( $hit, $repeatText, $nosummary,
                $showlock );
        }
        $result .= $outString;
    ...
    # print footer
    $afterText =
      Foswiki::Func::expandCommonVariables( $afterText, $topicName, $webName );
...
sub limit {
    ...
    my $maxlimit = $Foswiki::cfg{KinoSearchContrib}{MaxLimit} || 2000;
    if ( $maxlimit < $limit ) {
        $limit = $maxlimit;
    }
...
# i.e. 'search=x&web=System&web=Main', then query becomes 'x and (web:System web:Main)'
    if ( ( !$searchAllFlag ) && ($websStr) ) {
        my $searchStr = join ' web:', split( /[ ]+/, $websStr );
...
sub docsForQuery {
    ...
    if ( !$search ) {
        $search =
          "\"Something very unlikely to happen. Nothing to search for!\"";
    }
...
sub renderHtmlStringFor {
 ...
 #$comment =~ s/([\x{80}-\x{FFFF}])/'.'/gse; # FIXME bt now just get rid of UTF8
    ...
    if ( $hit->{author} ) {
        ...
        if ( $revUser !~ "$mainWebname." ) {
            $revUser = "$mainWebname.$revUser";
        }
        $revNum  = $hit->{version};
        $revDate = $hit->{date};
    }
    ...
    if ($name) {
        ...
        $icon = $name;
        $icon =~ s/["}%]//g
          ;   # ensure that the filename won't break the %ICON attribute parsing
        $icon = "%ICON{\"$icon\"}%";

        # URL for the file
        $tempVal =~
s/%MATCH%/<a href="%PUBURLPATH%\/$resweb\/$restopic\/$name">$name<\/a>/go;
        ...
        if ($showlock) {
            ( $url, $lockinguser, $locked ) =
              Foswiki::Func::checkTopicEditLock( $resweb, $restopic );
            if ($lockinguser) {
                $lockinguser =
                  Foswiki::Func::userToWikiName( $lockinguser, "0" );
            }
        }
    }
    ...
    $tempVal =
      Foswiki::Func::expandCommonVariables( $tempVal, $restopic, $resweb );
    ...
    if ($nosummary) {
        ...
        if ($name) {
            ...
            $tempVal =~
s/%TEXTHEAD%/\[\[$resweb\.$restopic\]\]$comment \[$hit->{excerpt}\]/go;
        }
        else {
...
#print "remoteUser = $remoteUser\n";
    my $allowView =
      Foswiki::Func::checkAccessPermission( "view", $remoteUser, $text,
        $restopic, $resweb );
    if ( !$allowView ) {
        return 0;
    }
